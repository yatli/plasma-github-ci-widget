...
Actions workflow as a shields-style badge, e.g. `[ ci | passing ]`.
...
- Status colors: green `passing`, red `failing`, yellow `running`/`queued`,
  grey `no status`/`cancelled`/`needs token`/`offline`.
...
| Field | Default | Notes |
|---|---|---|
| Repository | `ZGCI-SoftwareIntelligence/zgent` | `owner/repo` or full URL |
| Workflow file | `ci.yml` | empty = latest run of any workflow |
| Branch | *(empty)* | empty = default branch |
| Badge label | `ci` | left side text |
...
1. Repository access → **Only select repositories** → choose `ZGCI-SoftwareIntelligence/zgent`.
...
> `needs token` in grey for private repos.
